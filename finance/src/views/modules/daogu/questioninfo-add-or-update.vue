<template>
  <el-dialog
    ...
    <el-form-item label="问题状态（1待发布，2已发布3下架）" prop="status">
      <el-input v-model="dataForm.status" placeholder="问题状态（1待发布，2已发布3下架）"></el-input>
    </el-form-item>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '问题状态（1待发布，2已发布3下架）不能为空', trigger: 'blur' }
          ],
          auditStatus: [
    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/sys/daogu/question/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/sys/daogu/question/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
