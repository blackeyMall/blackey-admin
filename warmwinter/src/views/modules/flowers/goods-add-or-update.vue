  ...
  <el-dialog :title="!dataForm.id ? '新增' : '修改'" :close-on-click-modal="false" :visible.sync="visible">
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="商品描述" prop="goodsDesc">
        <el-input v-model="dataForm.goodsDesc" placeholder="商品描述"></el-input>
      </el-form-item>
    </el-form>
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/flowers/goods/info/${this.dataForm.id}`),
            method: "get",
        ...
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/flowers/goods/${!this.dataForm.id ? "save" : "update"}`
            ),
            method: "post",
